...
%% !!!À COMPLÉTER!!!
%%On check si c'est
expand(E, Eo) :- check(Env, E, T, Eo).
expand((X : E), Eo) :- app(apply(Env, X, E, E1), Eo). 
expand([X = E|Decls], Eo) :- let(Decls, E).
...
%%En appelant E3, on veut que X soit remplace par E4.
coerce(Env, forall(X, E2, E3), T, To, Eo) :-
    X1 = coerce(Env, forall(X, E2, E3), T1, To, Eo),
    coerce(Env, forall(X1, E2, E3), T1, T4, E4).
%%Conversition de int en float ou bool
coerce(_, E1, int, float, E2) :- E2 = app(int_to_float, E1).
coerce(_, E1, int, bool, E2) :- E2 = app(int_to_bool, E1).
...
%% !!!À COMPLÉTER!!!
%%2 regle
infer(Env, E1, E2, T) :- infer(Env, [X:E1|Env], E3, T),
			 infer(Env, fun(X, E1, E2), arw(X, E1, E3), T).
%%3 regle
infer(Env, (X:T), E2, T1) :- E1 = (X : T), E3 = infer(Env, E1, E2,  T1),
			       infer(Env, arw(X, E1, E3),fun(X, E2), T1).
%%4 regle
infer(Env, E1, E2, type) :- check(Env, arw(X, [X:E1|Env], E2), type, E2).
%%6 regle
%%Quand la fonction E5 est appelé dans l'appel récursif, on veut que X
%%remplace E2.
infer(Env, E1, E2, T) :- E1 = verify1(Env, app(X, E4, E5), type),
			 E5 = coerce(Env, E1, T, T, E),
			 infer(Env, app(E1, E2), E5, T).
%%8 regle
infer(Env, E1, E2, T) :- E3 = verify1(Env, let(X, T, E1, E2), Tret),
			 infer(Env, E3, E4, T).
...
%% !!!À COMPLÉTER!!!
check(Env, E1, T, Eo) :- infer(Env, E1, Eo, T1), equal(Env, T1, T).
